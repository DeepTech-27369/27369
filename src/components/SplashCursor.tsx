...
export default function SplashCursor({
  ...
  useEffect(() => {
    if (active) {
      ...
      const timer = setTimeout(() => setVisible(false), EXIT_DURATION);
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      const halfFloat = isWebGL2 ? (gl as WebGL2RenderingContext).HALF_FLOAT : (gl.getExtension("OES_texture_half_float") as any)?.HALF_FLOAT_OES;
      let formatRGBA: any;
      let formatRG: any;
      let formatR: any;

      if (isWebGL2) {
        formatRGBA = getSupportedFormat(gl, (gl as WebGL2RenderingContext).RGBA16F, gl.RGBA, halfFloat);
        formatRG = getSupportedFormat(gl, (gl as WebGL2RenderingContext).RG16F, (gl as WebGL2RenderingContext).RG, halfFloat);
        formatR = getSupportedFormat(gl, (gl as WebGL2RenderingContext).R16F, (gl as WebGL2RenderingContext).RED, halfFloat);
      } else {
    ...
    class Material {
      ...
      get uniforms() {
        return this.activeProgram?.uniforms ?? {};
      }
    }
    ...
    function addKeywords(source: string, keywords: string[] | null) {
      ...
      return keywords.map(kw => `#define ${kw}\n`).join('') + source;
    }
    ...
    const displayShaderSource = `
      ...
      void main () {
        ...
        #ifdef SHADING
          float lang = length(texture2D(uTexture, vR).rgb - texture2D(uTexture, vL).rgb);
          float tang = length(texture2D(uTexture, vT).rgb - texture2D(uTexture, vB).rgb);
          vec3 n = normalize(vec3(lang, tang, 2.0 * length(texelSize)));
    ...
    function generateColor() {
      ...
      c.r *= 0.6; // Increased brightness
      c.g *= 0.6; // Increased brightness
      c.b *= 0.6; // Increased brightness
      return c;
    }
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        ...
        if (!pointer) pointer = pointers[i] = pointerPrototype();
  ...
  return (
    <div
      className={`splash-cursor-container ${active ? 'is-active' : 'is-exiting'}`}
    >
      <canvas
        ref={canvasRef}
        style={{
