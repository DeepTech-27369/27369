...
export const LogoLoop = React.memo<LogoLoopProps>(
  ({
    ...
    const renderLogoItem = useCallback((item: LogoItem, key: React.Key) => {
      ...
      return (
        <li className="logoloop__item" key={key} role="listitem">{itemContent}</li>
      );
    }, []);
    ...
    const containerStyle = useMemo((): React.CSSProperties => ({ width: toCssLength(width) ?? "100%", ...cssVariables, ...style }), [width, cssVariables, style]);
